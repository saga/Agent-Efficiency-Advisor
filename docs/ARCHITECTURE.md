...
npm run demo:v4        # V4 实时推荐 + Shadow + Feedback
...
`npm run demo:v6` 跑 `src/cli-v6.ts`，生成 3 天 × 8 session × 76 event 的合成数据，依次过 5 层：
...
| `src/cli-v6.ts` | `npm run demo:v6` 端到端 demo |
...
| `src/cli-v6.ts` | `npm run demo:v6` 新增 Layer 6 demo（含 recover session 类型用于 Query 1 验证） |
...
| 修改 | `src/cli-v6.ts` | 传入 EntityBuilder 到 GraphBuilder，使用 `report.summary` |
...
npm run demo:v6         # 6-layer demo 通过：95 nodes / 121 edges / 11 sessions
